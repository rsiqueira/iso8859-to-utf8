...
# Description:
# Perl script to convert files from ISO-8859-1 to UTF-8 (Unicode).
# Useful to convert accents from old files to the new utf8 format.
# This will overwrite the original file with the converted strings.
#
# Usage:
# Converts strings from a single file:
# iso8859-to-utf8.pl filename.txt
#
# Converts multiple files at once:
# iso8859-to-utf8.pl filename1.txt filename2.txt filename3.txt ...
#
...
#
# I originally uploaded as a Gist Code snippet: https://gist.github.com/2884637 (old version)
#
# TODO: Should not convert again if the file was already converted (eg: test file content to see if it is already utf8)
...
foreach my $file (@files) {
  ...
  my $exe = "iconv -f ISO-8859-1 -t UTF-8 \"$file\" > \"$file_new_tmp\"";
  if ($debug > 1) {
    say "$exe";
  }
  my $ok = `$exe`; 

  say "Converting '$file' from iso-8859-1 to utf-8... ";
